...
{
    // 1) Get the hard-coded username
    const char *hardcoded_username = my_username();
    
    // 2) Obtain the dynamically allocated string from the conf file
    char *conf_file_username = malloc_username_from_conf_file();
    
    // 3) Verify that the two strings are equal
    TEST_ASSERT_EQUAL_STRING_MESSAGE(hardcoded_username, conf_file_username, 
                                     "The hard-coded username does not match the username in the conf file");
